...
def draw(solution):
    ...
    lines_num = len(solution)
    fig = plt.figure()
    for element in solution:
        new_list = [element[i] for i in range(len(element)) if element[i] is not None]  # 2
        print(new_list)
        y = [i[1] for i in new_list]
        x = [i[0] for i in new_list]
        # print("y:", y)
        # print("x:", x)
...
def read_cli_parameters():
    ...
    # parser.add_argument('ifile', help="Input CSV file path", default="SampleData/TestInput03.csv")
    # parser.add_argument('ofile', help="Output CSV file path", default="SampleData/output/03.csv")
    # parser.add_argument('pfile', help='Path to the parameter text file', default="params.txt")
    ...
    input_file = "SampleData/TestInput_03.csv"
    output_file = "SampleData/output/03.csv"
    ...
    print("return type:", type(solution_coordinates))
    print("info:", solution_coordinates)

    draw(solution_coordinates)
